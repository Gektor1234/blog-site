...
def register(request):    # функция для отоброжения страницы регистрации
    if request.method == "POST":
        form = UserOurRegistration(request.POST)
        if form.is_valid():
            form.save()
            username = form.cleaned_data.get('username')
            messages.success(request, f'Акаунт {username} был успешно создан, введите имя пользователя и пароль для авторизации')
...
def profile(request):    # фунуция для отображения профиля пользователя
    if request.method == "POST":
